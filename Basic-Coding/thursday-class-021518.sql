...
-- by the time that the ALTER TABLE changes were made for A) and B),
...
-- prep data for change request
-- some inventory data without itemDescription
INSERT INTO InventoryItems(ItemNumber,ItemDescription,CurrentSalePrice,InStockCount,ReorderLevel)
	VALUES('GR35A', NULL, 43.95, 8, 5)
INSERT INTO InventoryItems(ItemNumber,CurrentSalePrice,InStockCount,ReorderLevel)
	VALUES('KD45-Q', 43.95, 8, 5)
...
-- ENTER A BIT MORE INFO
INSERT INTO InventoryItems(ItemNumber,ItemDescription,CurrentSalePrice,InStockCount,ReorderLevel)
	VALUES('BL-92', '92mm Bolt', 45.95, 8, 5)

-- check data in the inventory table
SELECT ItemNumber, ItemDescription, CurrentSalePrice
	FROM InventoryItems
...
-- F) Data change requests: All inventory items that are less than 5.00 have to have ther prices increased by 10%
UPDATE InventoryItems
	SET CurrentSalePrice = CurrentSalePrice *0.10
	WHERE CurrentSalePrice < 5.0

-- somebody got married
UPDATE Customers
	SET LastName = 'Flinstone'
...
DELETE FROM InventoryItems
	WHERE ItemNumber IN ('GR47D', 'KDS-Q')
